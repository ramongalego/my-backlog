...
import { Layers, Inbox, Trophy, X, Ban, EyeOff } from 'lucide-react';
...
const FILTER_CONFIG: Record<GameFilter, { label: string; icon: ReactNode }> = {
  ...
  wont_play: { label: "Won't Play", icon: <Ban className="w-3.5 h-3.5" /> },
  hidden: { label: 'Hidden', icon: <EyeOff className="w-3.5 h-3.5" /> },
};

const FILTERS: GameFilter[] = ['all', 'backlog', 'finished', 'dropped', 'wont_play', 'hidden'];
